...
// Check for restricted content - make it less restrictive
function hasRestrictedContent(text) {
  // Only block obvious spam patterns, not all URLs/emails
  const spamRegex = /(http|https):\/\/(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+[a-z0-9][a-z0-9-]{0,61}[a-z0-9](?::[0-9]{1,5})?(?:\/.*)?/gi;
  const excessiveLinks = (text.match(spamRegex) || []).length > 2;
  
  return excessiveLinks;
}
...
exports.handler = async (event) => {
  ...
  try {
    ...
    console.error('Error in messages-get2:', error);
    return {
      statusCode: 500,
      headers: {
        'Access-Control-Allow-Origin': '*'
